...
distrobox enter "$BOX_NAME" -- bash -ic "
  command -v flutter &>/dev/null && flutter --version 2>/dev/null | head -1 | sed 's/^/  [ok] /' || echo '  [!!] flutter manquant'
  command -v dart &>/dev/null    && dart --version 2>&1 | sed 's/^/  [ok] /' || echo '  [!!] dart manquant'
  command -v gh &>/dev/null      && echo '  [ok] gh' || echo '  [!!] gh manquant'
" 2>/dev/null || true

echo ""
echo "Distrobox '$BOX_NAME' prête. Log : $LOG_FILE"
echo "Astuce : 'flutter doctor' pour vérifier l'environnement, 'flutter create mon_app' pour un nouveau projet."
echo "         Android SDK non installé par défaut — voir post_install.sh pour les détails."
