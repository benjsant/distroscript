...
# 3. uv
# uv remplace pyenv : binaires précompilés au lieu d'une compilation de CPython
# depuis les sources, soit ~2 secondes contre plusieurs minutes.
...
IA_PYTHON="3.${IA_PYTHON_MAX_MINOR}"
echo "Installation de Python $IA_PYTHON via uv..."
uv python install "$IA_PYTHON" --default
...
if [ ! -d "$IA_VENV" ]; then
  ...
  uv venv "$IA_VENV" --python "$IA_PYTHON"
fi
...
echo "Installation terminée. Python: $IA_PYTHON | PyTorch: mode $MODE"
